# Load a 10x Genomics MEX directory into a SingleCellExperiment

Reads the three-file MEX format produced by Cell Ranger
(`barcodes.tsv.gz`, `features.tsv.gz`, `matrix.mtx.gz`) using
DropletUtils.

## Usage

``` r
TALARIA_load_10x(
  path,
  sample_names = NULL,
  backend = c("memory", "bpcells"),
  bpcells_dir = NULL,
  verbose = TRUE
)
```

## Arguments

- path:

  Character. Path to the MEX directory (the folder containing the three
  Cell Ranger output files).

- sample_names:

  Character. Sample name(s) assigned to the `colData` `Sample` column.
  Defaults to the directory name.

- backend:

  Character. `"memory"` (default) or `"bpcells"`. See
  [`TALARIA_load_h5ad`](https://ylefol.github.io/L-Atelier/CAULDRON/reference/TALARIA_load_h5ad.md)
  for details.

- bpcells_dir:

  Character. Path for the BPCells on-disk store directory. Required when
  `backend = "bpcells"`.

- verbose:

  Logical. Print a summary. Default `TRUE`.

## Value

A `SingleCellExperiment` with a `"counts"` assay.
