...
AETHER_plot_part_wgcna_circos <- function(circos_data,
  ...
  if (show_legend) {
    ...
    # Module colors
    unique_modules <- unique(names(module_colors))
    legend_list[[length(legend_list) + 1]] <- ComplexHeatmap::Legend(
      labels = unique_modules,
      legend_gp = grid::gpar(fill = module_colors[unique_modules]),
      title = "WGCNA Modules",
      ncol = 2
    )

    # Cluster colors
    unique_clusters <- unique(names(cluster_colors))
    legend_list[[length(legend_list) + 1]] <- ComplexHeatmap::Legend(
      labels = unique_clusters,
      legend_gp = grid::gpar(fill = cluster_colors[unique_clusters]),
      title = "PART Clusters",
      ncol = 2
    )
