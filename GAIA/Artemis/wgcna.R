...
#'   where R² first exceeds this value. Default: 0.80.
#' @param mean_k_cutoff Optional. If R² criterion yields high mean connectivity,
...
#'   \item{r2_at_power}{R² value at selected power}
...
#' 1. WGCNA's built-in powerEstimate (if available and meets R² threshold)
#' 2. First power where scale-free topology R² exceeds r2_cutoff
#' 3. Power with best R² if none exceed threshold (with warning)
#'
#' The diagnostic plots show:
#' - Scale-free topology fit (R²) vs power
...
ARTEMIS_wgcna_pick_power <- function(wgcna_data,
  ...
  # Calculate signed R² (scale-free topology fit)
  ...
  if (power_auto) {
    ...
    # Method 2: First power above R² threshold
    if (is.na(selected_power)) {
      ...
      if (length(above_threshold) > 0) {
        ...
        if (verbose) {
          cat("Selected first power with R² >=", r2_cutoff, ":",
              selected_power, "\n")
        }
      }
    }
    ...
    if (is.na(selected_power)) {
      ...
      if (verbose) {
        warning("No power achieved R² >= ", r2_cutoff,
                ". Using best available: ", selected_power,
                " (R² = ", round(signed_r2[best_idx], 3), ")")
      }
    }
  ...
  if (!is.na(selected_power)) {
    recommendation <- sprintf(
      "Recommended power: %d (R² = %.3f, mean connectivity = %.1f)",
...
print.wgcna_power <- function(x, ...) {
  ...
  cat("R² at power:", round(x$r2_at_power, 3), "\n")
  cat("Mean connectivity:", round(x$mean_k_at_power, 1), "\n")
  cat("R² cutoff used:", x$r2_cutoff, "\n")
...
ARTEMIS_wgcna_detect_modules <- function(wgcna_data,
  ...
  # If another package (e.g., via org.Hs.eg.db) masks it, blockwiseModules fails.
  wgcna_cor_ok <- tryCatch({
    identical(environment(cor), asNamespace("WGCNA"))
  }, error = function(e) FALSE)

  if (!wgcna_cor_ok) {
    if (verbose) {
      message("Note: WGCNA's cor() function was masked by another package. ",
              "Restoring WGCNA::cor to global environment.")
    }
