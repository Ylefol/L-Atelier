...
#'
#' The original CIBERSORT.R writes a "CIBERSORT-Results.txt" file as a side
#' effect. This wrapper captures the file in a temporary directory and cleans
#' up after execution.
#'
#' @examples
...
ARTEMIS_cibersort <- function(mixture,
  ...
  if (is.character(mixture) && length(mixture) == 1 && file.exists(mixture)) {
    ...
    mixture_is_file <- TRUE
    if (verbose) cat("Using mixture file:", mixture, "\n")
    ...
    # Write to temp file (CIBERSORT expects tab-delimited file)
    tmp_dir <- tempdir()
    mixture_file <- file.path(tmp_dir, "cibersort_mixture_input.txt")
    utils::write.table(
      data.frame(Gene = rownames(mix_mat), mix_mat, check.names = FALSE),
      file = mixture_file,
      sep = "\t",
      row.names = FALSE,
      quote = FALSE
  ...
  # ---------------------------------------------------------------------------
  # Run CIBERSORT in a temporary working directory
  # (CIBERSORT writes "CIBERSORT-Results.txt" as a side effect)
  # ---------------------------------------------------------------------------
  run_dir <- tempfile("cibersort_run_")
  dir.create(run_dir)
  old_wd <- getwd()
  on.exit({
    setwd(old_wd)
