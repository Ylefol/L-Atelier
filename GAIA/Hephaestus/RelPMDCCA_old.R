    ...
    while ((diff > 1e-05) && (!is.na(diffTemp)) && (n.t < nIter)){
        ...
        while ((diffW_1 > 1e-05) && (!is.na(diffTemp)) && (n.t.w_1 < nIter)){
          ...
          for (j in 1:length(newW_1)){
              ...
              if (penalty == "LASSO") {
                  newW_1[j] <- proximal_lasso(omega[j], mu.x, c[j], tauW_1, currentW_1[j])
              } else if (penalty == "SCAD") {
                  newW_1[j] <- proximal_scad(omega[j], mu.x, c[j], tauW_1, a, currentW_1[j])
              } else if (penalty == "ELASTIC-NET") {
                  newW_1[j] <- proximal_elastic_net(omega[j], mu.x, c[j], tauW_1, tau_EN, currentW_1[j])
              } else {
        ...
        while ((diffW_2 > 1e-05) && (!is.na(diffTemp)) && (n.t.w_2 < nIter)){
          ...
          for (j in 1:length(newW_2)){
              # Use penalty functions from Minerva
              if (penalty == "LASSO") {
                  newW_2[j] <- proximal_lasso(omega[j], mu.y, c[j], tauW_2, currentW_2[j])
              } else if (penalty == "SCAD") {
                  newW_2[j] <- proximal_scad(omega[j], mu.y, c[j], tauW_2, a, currentW_2[j])
              } else if (penalty == "ELASTIC-NET") {
                  newW_2[j] <- proximal_elastic_net(omega[j], mu.y, c[j], tauW_2, tau_EN, currentW_2[j])
              } else {
                  stop("Please provide the penalty function to be either LASSO, SCAD, or ELASTIC-NET")
              }
          }
    ...
    for (i in 1:nIter){
        for (j in 1:length.list){
            ...
            for (k in 1:length(newW[[j]])){
                # Use penalty functions from Minerva
                if (penalty == "LASSO") {
                    newW[[j]][k] <- proximal_lasso(omega[k], mu.w[[j]], cc[k], tau[[j]], oldW[[j]][k])
                } else if (penalty == "SCAD") {
                    newW[[j]][k] <- proximal_scad(omega[k], mu.w[[j]], cc[k], tau[[j]], a, oldW[[j]][k])
                } else if (penalty == "ELASTIC-NET") {
                    newW[[j]][k] <- proximal_elastic_net(omega[k], mu.w[[j]], cc[k], tau[[j]], tau_EN, oldW[[j]][k])
                } else {
                    stop("Please provide the penalty function to be either LASSO, SCAD, or ELASTIC-NET")
                }
            }
...
#'
#' # Run RelPMDCCA
#' result <- relPMDCCA(X_1, X_2, lambda = 10, tauW_1 = 0.8, tauW_2 = 0.8, nIter = 100)
#'
