...
#'
#' @export
HEPHAESTUS_updateW_convCCA <- function(K, v, tau, penalty = "LASSO"){
    # Step 1 -- Initialize
    K <- as.matrix(K)
    v <- as.numeric(v)
    vx <- K %*% v
    ...
    if (penalty == "SCAD"){
        u.new <- MINERVA_soft_threshold_scad(vx, tau)
    } else if (penalty == "LASSO"){
        u.new <- MINERVA_soft_threshold_lasso(vx, tau)
    } else {
...
#'
#' @export
HEPHAESTUS_updateWmulti_convCCA <- function(K, w, whichDSet, tau, penalty = "LASSO"){
    if (!is.list(K)){
        stop("Please provide a valid K Correlation matrix. It should be a list")
    }

    # Step 1 -- Initialize
    # Initialize vx as NULL, will be set on first iteration
    vx <- NULL
    k_idx <- 1
    # Take the sum of canonical pairs to use in updating w
    for (i in 1:length(w)){
        if (i != whichDSet){
            K_curr <- as.matrix(K[[k_idx]])
            w_curr <- as.numeric(w[[i]])

            # Compute contribution
            contrib <- K_curr %*% w_curr

            # Add to vx (initialize on first iteration)
            if (is.null(vx)){
                vx <- contrib
            } else {
                vx <- vx + contrib
...
#' @keywords internal
#' @export
HEPHAESTUS_convCCA_algo <- function(X_1, X_2, tauW_1, tauW_2, initW_1 = NULL, initW_2 = NULL, nIter = NULL, penalty = "LASSO"){
    # Initializations
    X <- scale(X_1)
    Y <- scale(X_2)
    SigmaX <- cov(X)
    ...
    if (is.null(nIter)){
        ...
        while (diff > 1e-05 && !is.na(diffTemp)) {
            ...
            oldW_2 <- newW_2
            ## Update u:
            newW_1 <- HEPHAESTUS_updateW_convCCA(K = K, v = oldW_2, tau = tauW_1, penalty = penalty)
            ## Update v:
            newW_2 <- HEPHAESTUS_updateW_convCCA(K = t(K), v = newW_1, tau = tauW_2, penalty = penalty)
...
#' @keywords internal
#' @export
HEPHAESTUS_multi_convCCA_algo <- function(X, tau, nIter = 1000, penalty = "LASSO", initW = NULL){
    # Initializations
    length.list <- length(X)
    n.samples <- nrow(X[[1]])

    # Ensure tau is a list
    if (!is.list(tau)){
        if (is.numeric(tau) && length(tau) == length.list){
            tau <- as.list(tau)
        } else {
            stop("tau must be a list of length equal to number of datasets")
        }
    }

    # Validate tau structure
    if (length(tau) != length.list){
        stop(sprintf("tau must have length %d (number of datasets), got %d",
                     length.list, length(tau)))
    }
    ...
    for (k in 1:nIter){
        for (i in 1:length.list){
            ...
            for (j in 1:length.list){
                if (j != i){
                    K[[k_idx]] = SigmaXforK[[i]] %*% cov(X[[i]], X[[j]]) %*% SigmaXforK[[j]]
                    k_idx <- k_idx + 1
                }
            }
            wOld[[i]] <- wNew[[i]]
            ## Update u:
            wNew[[i]] <- HEPHAESTUS_updateWmulti_convCCA(K = K, w = wOld, whichDSet = i, tau = tau[[i]], penalty = penalty)
        }
    }
    return(list("w" = wNew))
}
...
#'
#' }
HEPHAESTUS_convCCA <-  function(X_1, X_2, tauW_1, tauW_2, initW_1 = NULL, initW_2 = NULL, nIter = NULL, penalty = "LASSO", R = 1){
    if (R == 1){
        values = HEPHAESTUS_convCCA_algo(X_1 = X_1, X_2 = X_2, tauW_1 = tauW_1, tauW_2 = tauW_2, initW_1 = initW_1, initW_2 = initW_2, nIter = nIter, penalty = penalty)
        finalW_1 <- values$w_1
        finalW_2 <- values$w_2

        # Check if all weights are zero (tau too high)
        all_zero <- all(abs(finalW_1) < 1e-10) && all(abs(finalW_2) < 1e-10)
        if (all_zero){
            warning("All canonical weights are zero across both datasets. This typically occurs when tau values are too high, causing excessive sparsity. Consider using lower tau values (e.g., 0.1-0.3 for initial exploration).")
        }
    } else if (R < 1){
        ...
        for (i in 1:R-1){
            ...
            values = HEPHAESTUS_convCCA_algo(X_1 = Xtilde_1, X_2 = Xtilde_2, tauW_1 = tauW_1, tauW_2 = tauW_2, initW_1 = initW_1, initW_2 = initW_2, nIter = nIter, penalty = penalty)
            tempW_1 <- values$w_1
            tempW_2 <- values$w_2
        }
        finalW_1 <- tempW_1
...
#'
#' @param X List of M datasets, where each element is an (n x p_i) matrix.
...
#'
#' }
HEPHAESTUS_multi_convCCA <- function(X, tau, nIter = 1000, penalty = "LASSO", initW = NULL, R = 1){
    if (R == 1){
        values = HEPHAESTUS_multi_convCCA_algo(X = X, tau = tau, nIter = nIter, penalty = penalty, initW = initW)
        finalW <- values$w

        # Check if all weights are zero (tau too high)
        all_zero <- all(sapply(finalW, function(w) all(abs(w) < 1e-10)))
        if (all_zero){
            warning("All canonical weights are zero across all datasets. This typically occurs when tau values are too high, causing excessive sparsity. Consider using lower tau values (e.g., 0.1-0.3 for initial exploration).")
        }
    } else if (R < 1){
        stop("Please provide a positive integer for the number of canonical pairs (R)")
    } else {
        ...
        for (k in 1:(R-1)){
            ...
            values = HEPHAESTUS_multi_convCCA_algo(X = Xtilde, tau = tau, nIter = nIter, penalty = penalty, initW = initW)
