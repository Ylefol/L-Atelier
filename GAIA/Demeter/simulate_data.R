...
#'     Each matrix has dimensions n_samples × n_features[i].}
...
#'   \item{shared_latent}{Matrix (n_samples × 1) containing the shared latent
...
#' Where:
#' \itemize{
#'   \item u is the shared latent variable (n_samples × 1)
#'   \item v_i is the canonical vector for dataset i (n_features[i] × 1)
...
DEMETER_generate_multi_datasets <- function(n_datasets = 3,n_samples = 100,n_features = c(500, 1000, 750),signal_samples = 25,signal_features = c(50, 50, 40),seed = 1234) {
  ...
  for (i in 1:length(simulation_list$datasets)) {
    cat(sprintf("  Dataset %d: %d samples × %d features (signal in first %d features)\n",
